...
- preserve the first-seen provider record while filling its missing fields from
  duplicate records returned by later providers;
...
For scalar metadata such as `abstract`, `doi`, `journal`, `year`, `url`, and
...
existing `source` field for older records. All other report fields and the
current 300-character abstract preview remain unchanged.
...
Result enrichment is conservative: only missing scalar fields are filled, so a
lower-priority provider cannot silently replace existing metadata from the
primary provider.
...
report under `reports/`. Verification will confirm the file exists, contains
results, renders complete author lists, and includes abstracts when returned
by the contributing providers. Live provider availability is reported as
observed evidence rather than treated as a deterministic unit-test condition.
